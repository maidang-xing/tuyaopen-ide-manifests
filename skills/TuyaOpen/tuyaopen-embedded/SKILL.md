...
| Get a **third-party** library into the project — find it, install it, record it | `tuyaopen-cli dependency search` · `dependency install` · `dependency add` · `dependency list` · `dependency remove` | `tuyaopen-workflow-embedded-dev` (see below) |
...
让编译器真正看见这个库的最后一步没有命令，参见 `tuyaopen-workflow-embedded-dev`
